...
chennai (chen & ai) is a hybrid AI agent and a terminal user interface for exploring graphs produced by analysis tools such as AppThreat [atom](https://github.com/AppThreat/atom) and [rusi](https://github.com/cdxgen/cdxgen-plugins-bin/tree/main/thirdparty/rusi). It is built on top of the [chen](https://github.com/AppThreat/chen) library (Code Hierarchy Exploratory Network) and provides a keyboard-driven interactive environment for static analysis, data-flow tracing, and AI-assisted code and security analysis.
...
- Interactive exploration of Rust codebases using rusi analysis reports. Browse declarations, imports, usages, call graphs, data flows, and cryptographic evidence with dedicated rusi tools.
...
- For Rust analysis: a rusi binary. Rusi is bundled with cdxgen-plugins-bin or can be built from source. Set `RUSI_CMD` to point to the binary.
- Java 23 or newer if running the engine via the JAR fallback distribution (atom mode only).
- At least 4GB of available memory for the TUI and engine together. Larger codebases may need more. Rust analysis via rusi does not require the Scala engine.
